...
export class GeminiService {
  ...
  async generateLessonPlan(prompt: string): Promise<Planejamento | { rawText: string }> {
    ...
    const fullPromptWithCurriculumContext = `
      ...
      **Aulas do Currículo Existente (para referência e sugestão de adequação):**
      ...
      Gere a resposta EXCLUSIVAMENTE como um objeto JSON válido, aderindo estritamente à seguinte estrutura:
    ...
    } else if (lastError && lastError.message.includes('JSON.parse')) {
          return { rawText: lastError.input || 'Resposta da IA não foi um JSON válido.' };
    }
  ...
  async chatWithLessonPlan(prompt: string): Promise<{ updatedPlan?: Planejamento; rawText?: string }> {
    ...
    if (lastError && lastError.message.includes('503 Service Unavailable')) {
        ...
        throw new InternalServerErrorException('Não foi possível encontrar um modelo de IA disponível para chat.');
            ...
            return { rawText: lastError.input || 'Resposta da IA para chat não foi um JSON válido.' };
    }
